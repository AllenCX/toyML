...
test[['shot_id', 'shot_made_flag']].to_csv('sub_xgb_test.csv', index=False)
...
# In[ ]:
print("now running GridSearch")
predictors = [x for x in X_train.columns if x not in target]
print("now run the GridSearchCV")
param_test1 = [{
 'max_depth':[9,10,11],
...
gsearch1 = GridSearchCV(estimator = xgb.XGBClassifier(n_estimators=200,
 objective= 'binary:logistic', nthread=16, scale_pos_weight=1, seed=21), 
...
gs_result.to_csv("gs_result_xgb_test.csv")